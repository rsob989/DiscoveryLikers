...
import pl.project.weekop.service.VoteService;
...
public class VoteController extends HttpServlet {
	...
	private void updateVote(long userId, long discoveryId, VoteType voteType) {
		...
		Vote existingVote = voteService.getVoteByDiscoverUserId(discoveryId, userId);
	...
	private void updateDiscovery(long discoveryId, Vote existingVote, Vote updatedVote) {
		...
		if(existingVote==null && updatedVote!=null) {
			updatedDiscovery = addDiscoveryVote(discoveryById, updatedVote.getVoteType());
		} else if(existingVote!=null && updatedVote!=null) {
			updatedDiscovery = removeDiscoveryVote(discoveryById, existingVote.getVoteType());
			updatedDiscovery = addDiscoveryVote(updatedDiscovery, updatedVote.getVoteType());
		}
